...
source("https://raw.githubusercontent.com/ramirobentes/NBA-in-R/master/2016_17/add_data_pbp.R")

player_logs <- nba_leaguegamelog(season = "2016-17", player_or_team = "P") %>%
  pluck("LeagueGameLog") %>%
  clean_names() %>%
...
pbp_month <- read_csv("https://github.com/ramirobentes/NBA-in-R/releases/download/pbp17to22/data.csv",
                      col_types = c(cl = "c")) %>%
  clean_names() %>%
  filter(str_sub(game_id, 1, 4) == 2160)
