...
source("https://raw.githubusercontent.com/ramirobentes/NBA-in-R/master/2018_19/add_data_pbp.R")

player_logs <- nba_leaguegamelog(season = "2018-19", player_or_team = "P") %>%
  pluck("LeagueGameLog") %>%
  clean_names() %>%
...
starters_quarters <- nba_pbp %>%
  ...
  left_join(player_logs %>%
              distinct(game_id = as.integer(game_id), player_name, slug_team = team_abbreviation)) %>% bind_rows(missing_starters)
...
other_1ft_fouls <-  lineup_game %>%
  filter(msg_type == 3 & act_type == 10) %>%
  select(game_id, secs_passed_game, number_event_ft = number_event, slug_team) %>%
  ...
  left_join(lineup_game %>%
              filter(msg_type == 6,
                     str_detect(description, "1 FTA"),
                     !str_detect(description, "Flagrant|Technical|3 Second")) %>%
              transmute(game_id, secs_passed_game, number_event_foul = number_event, 
                        slug_team = ifelse(slug_team == team_home, team_away, team_home)) %>%
              group_by(game_id) %>%
              group_by(game_id, secs_passed_game, slug_team) %>%
...
other_fouls <- lineup_game %>%
  ...
  anti_join(other_1ft_fouls %>%
              select(game_id, number_event_ft, number_event_foul) %>%
              pivot_longer(cols = starts_with("number"),
                           names_to = "type_event",
                           values_to = "number_event") %>%
              select(-type_event))
...
fouls_stats <- bind_rows(regular_fouls, flagrant_clear, techs, other_1ft_fouls) %>%
