...
source("https://raw.githubusercontent.com/ramirobentes/NBA-in-R/master/2019_20/add_data_pbp.R")

player_logs <- nba_leaguegamelog(season = "2019-20", player_or_team = "P") %>%
  pluck("LeagueGameLog") %>%
  clean_names() %>%
  mutate(team_location = ifelse(str_detect(matchup, "\\@"), "away", "home"),
         across(c(player_id, team_id), as.numeric))

pbp_month <- read_csv("https://github.com/ramirobentes/NBA-in-R/releases/download/pbp17to22/data.csv",
                      col_types = c(cl = "c")) %>%
  clean_names() %>%
  filter(str_sub(game_id, 1, 4) == 2190)
...
# }
# missing_starters_new <- map2_df(qtrs_missing$game_id, qtrs_missing$period, missing_st_qtr)
...
techs <- lineup_game %>%
  ...
  filter(str_detect(description, "Technical|Defense 3 Second") & !str_detect(description, "Double Technical|Ejection:Second Technical")) %>%
...
regular_fouls <- other_fouls %>%
  #filter(game_id == 21900622, period == 3) %>%
  filter(msg_type == 3) %>%
...
regular_fouls <- regular_fouls %>%
  ...
  distinct(game_id, number_event_ft, .keep_all = TRUE)
...
pbp_final_gt <- pbp_poss_final %>%
  ...
  rows_update(manual_changes, by = c("game_id", "number_event"))
