...
corrections <- readxl::read_excel("nba_com adv box.xlsx", sheet = "corr pbp hoopR",
                                  col_types = c(clock = "text")) %>%
  mutate(across(c(game_id, secs_passed_game, number_event, msg_type, act_type), as.numeric))

player_logs <- nba_leaguegamelog(season = "2021-22", player_or_team = "P") %>%
  pluck("LeagueGameLog") %>%
  clean_names() %>%
  mutate(across(contains("id"), as.integer),
         team_location = ifelse(str_detect(matchup, "\\@"), "away", "home"))
...
nba_com_pbp <- read_csv("pbp_nba_api_22.csv",
                        col_types = c(cl = "c")) %>%
  filter(game_id != 0022100744) %>% # ta faltando 4o quarto na funcao nba_data_pbp (ver depois se colocam)
  ...
  left_join(player_logs %>%
              distinct(game_id, slug_team = team_abbreviation, team_location) %>%
              pivot_wider(names_from = team_location,
                          values_from = slug_team,
                          names_prefix = "team_"))

nba_com_pbp <- nba_com_pbp %>%
...
starters_quarters <- nba_com_pbp %>%
  ...
  left_join(player_logs %>%
              distinct(game_id, player_name, slug_team = team_abbreviation))
...
# add players that played entire quarter and didn't contribute to pbp
missing_starters <- readxl::read_excel("nba_com adv box.xlsx", sheet = "Missing") %>%
  select(game_id = idGame, period = numberPeriod, slug_team = slugTeamPlayer, player_name = namePlayer)

# put together table with starter for every team in every period
...
write_csv(lineup_game, "lineup_game_hoopr.csv")


# Possession --------------------------------------------------------------

lineup_game <- read_csv("lineup_game_hoopr.csv",
                        col_types = c(clock = "c"))
...
# conecting free throws to fouls
...
fouls_stats <- bind_rows(regular_fouls, flagrant_clear, techs) %>%
  ...
  # filter(is.na(shot_pts))  # teste pra ver se nao falta nada
...
pbp_poss_final <- pbp_poss %>%
  mutate(possession = ifelse(start_poss == "00:00.0", 0, possession)) %>%   # considerando erros da nba.com quando jogada ta com relogio errado em 00:00.0 (correto seria nao ter essa linha)
...
lineup_stats <- pbp_poss_final %>%
  ...
  select(game_id, period, stint, number_event, msg_type, description, lineup, pts_team, pts_opp,
         poss_team, poss_opp, secs_played, slug_team, slug_opp) %>%
  group_by(game_id, period, stint, slug_team, slug_opp, lineup) %>%
  summarise(across(c(pts_team, pts_opp, poss_team, poss_opp, secs_played), sum)) %>%
  ungroup()
# start and final time/score/poss can be done by grouping by game and using cumsum

# write_csv(pbp_poss_final, "pbp poss hoopr.csv")
...
## loading official nba.com possessions by team/quarter
funct_poss_period <- function(x){
  nba_teamgamelogs(season = "2021-22", measure_type = "Advanced", team_id = "", period = x) %>%
    pluck("TeamGameLogs") %>%
    clean_names() %>%
    transmute(across(c(game_id, poss), as.integer), game_date = as.Date(game_date), team_abbreviation,
              period = x) %>%
    select(game_date, game_id, period, slug_team = team_abbreviation, poss)
}
off_poss_period <- map_df(1:7, funct_poss_period)

compare_poss <- lineup_stats %>%
  group_by(game_id, period, slug_team) %>%
  summarise(poss = sum(poss_team)) %>%
  ungroup() %>%
  left_join(off_poss_period %>%
              rename(poss_nba = poss)) %>%
  select(game_date, game_id, period, slug_team, poss, poss_nba) %>%
  arrange(game_id)

compare_poss %>%
  count(diff = poss - poss_nba)


# Plus Minus e Minutes
player_stats <- lineup_stats %>%
  separate_rows(lineup, sep = ", ") %>%
  group_by(game_id, period, player_name = lineup) %>%
  summarise(across(c(pts_team, pts_opp, secs_played), sum)) %>%
  ungroup() %>%
  mutate(plus_minus = pts_team - pts_opp)

funct_player_period <- function(x){
  nba_playergamelogs(season = "2021-22", player_id = "", period = x) %>%
    pluck("PlayerGameLogs") %>%
    clean_names() %>%
    transmute(across(c(game_id, plus_minus), as.integer), game_date = as.Date(game_date), player_name, 
              slug_team = team_abbreviation, period = x)
}
off_player_period <- map_df(1:7, funct_player_period)

compare_pm <- off_player_period %>%
  rename(off_plus_minus = plus_minus) %>%
  left_join(player_stats) %>%
  mutate(min_played = secs_played / 60)

compare_pm %>%
  filter(off_plus_minus != plus_minus)



# Borrao ------------------------------------------------------------------

# procurando erros no poss count
pbp_poss %>%
  filter(game_id == 22100331,
         period == 2,
         possession == 1,
         off_slug_team == "PHX") %>%
  left_join(player_logs %>% distinct(game_id, game_date)) %>%
  select(game_date, clock, msg_type, act_type, description, possession, start_poss) %>%
  mutate(total = cumsum(possession)) %>%
  tail(15)
...
off_player_stats <- hoopR::nba_leaguedashplayerstats(season = "2021-22") %>%
  pluck("LeagueDashPlayerStats") %>%
  select(player_name = PLAYER_NAME, slug_team = TEAM_ABBREVIATION, games = GP, season_minutes = MIN, season_pm = PLUS_MINUS) %>%
  mutate(across(c(games:season_pm), as.numeric))

off_player_stats %>%
  left_join(off_player_period %>%
              group_by(player_name) %>%
              summarise(total_pm = sum(plus_minus))) %>%
  filter(season_pm != total_pm)

off_player_stats %>%
  left_join(player_stats %>%
              group_by(player_name) %>%
              summarise(total_secs = sum(secs_played),
                        total_pm = sum(plus_minus))) %>%
  mutate(total_min = total_secs / 60) %>%
  #count(difference = season_minutes - min_played) %>%
  #arrange(-difference)
  filter(season_pm != total_pm)

player_stats %>%
  filter(player_name == "Alec Burks") %>%
  summarise(total_pm = sum(plus_minus))
...
off_player_period %>%
  distinct(period, game_id) %>%
  anti_join(player_stats %>%
              distinct(period, game_id))
